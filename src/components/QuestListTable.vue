...
const allItems = shallowRef<QuestDataItem[]>([])
const sortedItems = shallowRef<QuestDataItem[]>([])
...
const filteredItems = computed(() => {
  ...
  if (appliedSearchQuery.value) {
    result = result.filter(
      (item) =>
        String(item.QuestId).toLowerCase().includes(appliedSearchQuery.value) ||
        (item.QuestName && item.QuestName.toLowerCase().includes(appliedSearchQuery.value)),
    )
  }
...
<template>
  <div class="text-card-foreground shadow-sm p-0 space-y-4">
    <div class="flex items-center pb-8 gap-2">
      <Input
        id="quest-data-search"
        :disabled="isLoading"
        v-model="searchQuery"
    ...
    >
      <el-table-column prop="QuestId" label="QuestId" sortable="custom" min-width="120" />
      <el-table-column prop="QuestName" label="Quest Name" sortable="custom" min-width="200" />
      <template #empty>
