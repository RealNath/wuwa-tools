...
self.onmessage = async (event) => {
    ...
    if (command == 'fetch_data') {
        ...
        const languages: string[] = ["de", "en", "es", "fr", "ja", "ko", "pt", "th", "zh-Hans", "zh-Hant"]
        let data: any
        let jsonDataEn = await fetchData("multitext", version, "en")
        
        type oLangDict = {
            [key: string]: string
        }
        let oLangList: oLangDict[] = []

        // add the 'g' flag to the regex to find ALL occurrences globally!
        const idRegex = new RegExp(`"Id":\\s*"([^"]+)",\\s*"Content":\\s*"${input}"`, 'g')
        
        // use matchAll to get every single match
        ...
        for (const lang of languages) {
            try {
                ...
                for (let i = 0; i < ids.length; i++) {
                    ...
                    const contentRegex = new RegExp(`"Id":\\s*"${currentId}",\\s*"Content":\\s*"((?:[^"\\\\]|\\\\.)*)"`)
                ...
                const wikiTexts = oLangList.map((dict, index) => {
                    return {
                        id: ids[index],
